...
if len(sys.argv) != 3:
	sys.exit("usage: presentator.py datafilename1 datafilename2   \n")
...
for i in range(n):
	if i in list_solutions:
		abscissa_sol.append(list_vol[i])
		ordinate_sol.append((1+list_mu[i])**250-1)
	else:
		abscissa_notsol.append(list_vol[i])
		ordinate_notsol.append((1+list_mu[i])**250-1)


plt.plot(abscissa_notsol,ordinate_notsol,'rx',label='not in the portfolio')
plt.plot(abscissa_sol,ordinate_sol,'bx',label='in the portfolio')
plt.plot(port_vol,(1+port_ret)**250-1,'gx',label='portfolio')
plt.xlabel('volatility')
plt.ylabel('average annualized rate of return')
plt.title('Plot of the average of annualized return as a function of the volatility for each asset\nFor '+str(n)+' assets')
plt.legend()
plt.savefig('plotan.png',bbox_incher='tight')